...
Future<Stream<Restaurant>> getNearRestaurants(LocationData myLocation, LocationData areaLocation, String resCatId) async {
  ...
  final String url = '${GlobalConfiguration().getString('api_base_url')}restaurants?$_apiToken$_nearParams&res_category_id=$resCatId';//$_orderLimitParam';
  print('get Top Restaurants:$url');
...
Future<Stream<AppBanner>> getBanner(String resCatId) async {
  User _user = await getCurrentUser();
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}banners?$_apiToken'; //&res_category_id=$resCatId';
  print('getBanner: $url');
  final client = new http.Client();
  final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
  return streamedRest.stream
    ...
    print('getbannerResponse:${jsonMap.toString()}');*/
    return AppBanner.fromJSON(data);
  });
}
Future<Stream<Offer>> getOffer() async {
  User _user = await getCurrentUser();
  final String _apiToken = 'api_token=${_user.apiToken}';
  final String url =
      '${GlobalConfiguration().getString('api_base_url')}offer?${_apiToken}';
  print('getOffer:$url');
  final client = new http.Client();
  final streamedRest = await client.send(http.Request('get', Uri.parse(url)));
  return streamedRest.stream
