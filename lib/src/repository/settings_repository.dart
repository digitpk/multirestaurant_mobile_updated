...
ValueNotifier<Locale> locale = new ValueNotifier(Locale('he', ''));
...
Future<Setting> initSettings() async {
  ...
  if (response.statusCode == 200 && response.headers.containsValue('application/json')) {
    print('Setting response:${json.decode(response.body)['data']}');
...
Future<Setting> getCurrentSettings() async {
  ...
  SharedPreferences.getInstance().then((SharedPreferences prefs) {
    if (prefs.containsKey('settings')) {
      _setting = Setting.fromJSON(json.decode(prefs.get('settings')));
      print('get setting:${_setting.toMap()}');
      setting.value = _setting;
    } else {
